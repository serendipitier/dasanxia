...
BOOL CEnumObjDlg::OnInitDialog()
{
	...
	ShowMsg("First, you need connect to token!\r\n");
...
void CEnumObjDlg::OnBtnConnect() 
{
	...
	if(CKR_OK != rv )
	{
		ShowErr(NEWLINE"Can't acquire the information of token, ErrorCode: 0x%08X.\r\n", rv);
		return;
	}
	if(0 >= ulCount)
	{
		ShowMsg(NEWLINE"Can't connect to token, make sure one USB Token has been inserted.\r\n");
		return;
	}
	...
	if (! m_pSlotList) 
	{
		ShowMsg(NEWLINE"Can't allocate enough memory!\r\n");
		return;
	}
	...
	if(CKR_OK != rv )
	{
		ShowErr(NEWLINE"Can't Connect to token, Error Code 0x%08X.\r\n", rv);
		delete[] m_pSlotList;
		m_pSlotList = NULL_PTR;
	}
	else
	{
		ShowMsg(NEWLINE"Connect to token Successfully!\r\n");
		m_btnConnect.EnableWindow(FALSE);
		ShowMsg(NEWLINE"Now You need to login with User PIN!\r\n");
...
*/
	...
	{
		StartOP();
		ShowMsg(NEWLINE"No certificate object.\r\n");
	}
	else
	{
		StartOP();
		...
		ShowMsg(NEWLINE);
		ShowMsg(strNumObj);
		ShowMsg(" certificate object(s) was found.\r\n");
	}
}
...
void CEnumObjDlg::OnBtnLogin() 
{
	...
	if (CKR_OK != rv)
	{
		ShowMsg(NEWLINE"Can not get token info!\r\n");
		return;
	}
	...
	if("" == g_strUserPIN)
	{
		ShowMsg(NEWLINE"To enum all objects, you must input User PIN!\r\n");
		return;
	}
	...
	if(CKR_OK != rv)
	{
		ShowErr(NEWLINE"Can't login to token, ErrorCode: 0x%08X.\r\n", rv);
		if(rv==CKR_USER_ALREADY_LOGGED_IN)
			ShowErr(NEWLINE"You have already logged in!",NULL);
		return;
	}
	else
		ShowMsg(NEWLINE"Login to Token Successfully!\r\n");
...
void CEnumObjDlg::OnBtnData() 
{
	...
	{
		StartOP();
		ShowMsg(NEWLINE"No data object.\r\n");
	}
	else
	{
		StartOP();
		...
		ShowMsg(NEWLINE);
		ShowMsg(strNumObj);
		ShowMsg(" data object(s) was found.\r\n");
	}
...
void CEnumObjDlg::OnBtnPublic() 
{
	...
	{
		StartOP();
		ShowMsg(NEWLINE"No public key object.\r\n");
	}
	else
	{
		StartOP();
		...
		ShowMsg(NEWLINE);
		ShowMsg(strNumObj);
		ShowMsg(" public key object(s) was found.\r\n");
	}
...
void CEnumObjDlg::OnBtnPrivate() 
{
	...
	{
		StartOP();
		ShowMsg(NEWLINE"Can's find private key Obj\r\n");
	}
	else
	{
		StartOP();
		...
		ShowMsg(NEWLINE);
		ShowMsg(strNumObj);
		ShowMsg(" private key object(s) was found.\r\n");
	}
}
...
void CEnumObjDlg::OnBtnSecret() 
{
	...
	else
	{
		...
		ShowMsg(NEWLINE);
		ShowMsg(strNumObj);
		ShowMsg(" secret key object(s) was found.\r\n");
	}
}
